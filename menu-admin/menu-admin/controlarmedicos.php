...
$sql = "SELECT * FROM usuarios WHERE tipo = 'm' ORDER BY id LIMIT $id, $total";

//Conta a quantidade total de registros
$sql1 = "SELECT count(*) as contagem FROM usuarios WHERE tipo = 'm'";
        ...
        <table id = "Tabela-Usuarios">
            ...
            <tr>
            	...
            	<?php
            	  while($row = $result->fetch_assoc()) {
            		...
            		$row["nome"] . "</td><td>" .
            		$row["email"] . "</td><td>" .
            		$row["especialidade"] . "</td><td>" .
            		$row["data_entrada"] . "</td><td>" .
            		$row["faculdade"] . "</td><td>" .
            		$row["consulta"] . "</td><td>" .
            		$row["tipo"] . "</td><td>" .
            		$row["status"];
           ...
           <div class="pagination" style="margin-left: 80px">
                <?php for($i=1; $i <= $contagem; $i++) {
                        echo "<a href='controlarmedicos.php?pag=$i'>$i</a>";
                } 
            	?>   
            </div>  
            </div> 
